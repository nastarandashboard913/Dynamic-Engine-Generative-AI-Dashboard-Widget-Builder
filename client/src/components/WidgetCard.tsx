...
export function WidgetCard({
  ...
  return (
    ...
    >
      {hasHeader && (
        <header className="flex items-start justify-between gap-3 px-4 pt-4 pb-3 sm:px-5">
          <div className="min-w-0">
            {title && (
              <h3 className="truncate text-title font-semibold tracking-tight text-text">
                {title}
              </h3>
            )}
            {subtitle && (
              <p className="mt-0.5 truncate text-mini text-text-muted">{subtitle}</p>
            )}
          </div>
          ...
          <div className="flex shrink-0 items-center gap-1.5 text-text-dim">
            ...
            {verified && (
              <CheckCircle2
                ...
                className="size-icon text-success"
                strokeWidth={2}
              />
            )}
            {envelope && (
      ...
      <div className={cn('min-h-0 flex-1', hasHeader ? 'px-4 pb-4 sm:px-5 sm:pb-5' : 'p-4 sm:p-5', bodyClassName)}>
        {children}
      </div>
    </section>
  )
